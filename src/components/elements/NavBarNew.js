...
// Analytics
import { ReactComponent as Logo } from '../../assets/images/DashWatch.svg';
...
class NavBar extends React.Component {
    ...
    //    history.pushState(null, null, `/proposals?search=${event.target[0].value}`)
        window.location.replace(`/proposals?search=${event.target[0].value}`)
        trackSearch('Searched: ' + event.target[0].value)
        this.setState({
            newPath: `/proposals?search=${event.target[0].value}`,
        })
    }
